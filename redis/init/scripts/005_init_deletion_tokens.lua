...
redis.call("HSET", "deletion_token_config",
...
local function delete_deletion_token(user_id)
    ...
    if redis.call("EXISTS", token_key) == 1 then
        ...
        -- Update statistics
        redis.call("HINCRBY", deletion_token_stats_key, "active_tokens", -1)
        redis.call("HINCRBY", deletion_token_stats_key, "used_tokens", 1)
...
local function cleanup_expired_tokens()
    ...
    local expired_tokens = redis.call("ZRANGEBYSCORE", deletion_token_cleanup_key, 0, current_time)
